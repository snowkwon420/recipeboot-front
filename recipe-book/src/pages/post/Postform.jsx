...
function Postform () {
    ...
    navigate(`/postingredients?title=${encodeURIComponent(title)}&contents=${encodeURIComponent(sanitizedContents)}`);
  }
...
// 페이지 새로고침 혹은 창닫기 시도시 경고 메세지 출력
	useEffect(() => {
		const handleBeforeUnload = (event) => {
			...
			event.returnValue = '변경사항이 저장되지 않을 수 있습니다. 정말 페이지를 나가시겠습니까?';
		};
	...
	return(
			...
			<div className='recipebox'>
				...
				<div>레시피 내용</div>
				<ReactQuill value={contents} onChange={(value) => onChange('contents', value)} />
				<hr/>
